...
function doProduct (id , name , price) {
	...
	if (bgcolor == '#FFFFFF') {
		...
		var html = '<li class="ccbg2" id="'+wemallId+'"><div class="orderdish"><span name="title">'+name+'</span><span class="price" id="v_0" style="display:none;">'+price+'</span><span style="display:none; class="price">元</span></div><div class="orderchange" style="display:none"><a href=javascript:addProductN("'+wemallId+'_'+price+'") class="increase"><b class="ico_increase">加一份</b></a><span class="count" id="num_1_499">1</span><a href=javascript:reduceProductN("'+wemallId+'_'+price+'") class="reduce"><b class="ico_reduce">减一份</b></a></div></li>';
...
function submitOrder () {
	...
	$('#ullist li').each(function () {
		...
		json += '{"name":"'+name+'","num":"'+num+'","price":"'+price+'","did":"'+did+'"},';
	...
	*/
	$.ajax({
		type : 'POST',
		url : appurl+'/App/Qinghuo/addorder',
		data : {
			uid : $_GET['uid'],
			cartData : json,
			userData : $('form').serializeArray(),
