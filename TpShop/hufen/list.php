...
if($uid){
	...
	$callback ="/index.php?g=App&m=Index&a=member&callback=hufen/list.php";
	...
	<ul id="bottom2">
		...
		<ul class="list"> 
		 ...
		 <div id="gdzd">
           ...
           <br/><a href="/index.php?g=App&m=Index&a=zhiding" style="padding: 5px; background-color: #EA222E; color: #Fff; border-radius: 5px; position: relative; left: 50%; margin-left: -14%;">开通独享置顶</a><br/><br/>
		<?php
				//显示后台排序列表
			...
			<li>
				...
				//显示shuaxin列表
			$query = mysql_query("select * FROM `weixin` where  listorder=0 and (".time()."-uptime)<=60 ORDER BY `shuaxin` DESC") or die(mysql_error());
				while($row=mysql_fetch_array($query)){
				?>
			...
			<li>
				...
				<div class="desc">
					<span class="name"><span style="color: #999;">[<?=$row['prov']?><?=$row['city']?>]</span><?=$row['name']?></span>
					<span class="desc_info"><?=$row['miaoshu']?></span>
					<span style="background:red;border-radius:3px;color:#fff;font-size:12px;padding:2px;padding-right:4px;margin-left:4px;" >VIP置顶剩余:<i class="top_time"><?php echo 60-(time()-$row['uptime']);?></i>秒</span>
				</div>
				<div class="adddiv">
					<a href="javascript:;" class="addfans" fansid="<?=$row['id']?>">加好友</a>
				</div><input type="hidden" name="fsimg<?=$row['id']?>" class="fscode_it<?=$row['id']?>" value="<?=$row['codeimg']?>" />
			...
			//---------END--------
				...
				$sj1 = substr($utime ,-4);
				$sj2 = substr($utime ,-3);
				$sql="SELECT *,mod(id*{$sj1} , {$sj2}) as sjpx  FROM weixin where shuaxin=0 and listorder=0 $where ORDER by sjpx DESC LIMIT 0,15";
				$query=mysql_query($sql);
				while($row=mysql_fetch_array($query)){
				?>
