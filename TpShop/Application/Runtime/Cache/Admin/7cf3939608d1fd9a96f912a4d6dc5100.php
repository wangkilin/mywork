							...
							<div class="col-xs-12">
								<div class="table-responsive">
									<table id="sample-table-1"
										...
										<?php
 foreach ($menu as $key => $value) { echo '<br/><br/>顶级：<input type="text" class="input" name="menu_name"
														value="'.$value['menu_name'].'" id="name" ><a href="javascript:void(0);" onclick="reWxmenu('.$value['menu_id'].');" class="btn btn-white btn-sm">修改</a>'; $menu2 = M ( "Wxmenu" )->where("pid=".$value['menu_id'])->order('listorder asc')->select (); foreach ($menu2 as $key => $value) { echo '<br/>&nbsp&nbsp&nbsp&nbsp∟<input type="text" class="input" name="menu_name"
														value="'.$value['menu_name'].'" id="name" ><a href="javascript:void(0);" onclick="reWxmenu('.$value['menu_id'].');" class="btn btn-white btn-sm">修改</a>'; ?><a class="J_ajax_del btn btn-white btn-sm"  href="javascript:drop_confirm('确定要删除这个用户吗？','<?php echo U('Admin/Weixin/delmenu',array('id'=>$value['menu_id']));?>')">删除</a><?php
 } } ?>
											...
											<tbody>
												<tr>
													...
													<td><select name="pid" class="normal_select">
													      ...
													      <?php if(is_array($menu)): $i = 0; $__LIST__ = $menu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menu): $mod = ($i % 2 );++$i;?><option value="<?php echo ($menu["menu_id"]); ?>"><?php echo ($menu["menu_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
		...
		<script type="text/javascript">
			function reWxmenu(o){
				var menu_id = o;
				$.post("<?php echo U('Admin/Weixin/getwxid');?>",{"menu_id":menu_id},function(data){
					var json = eval(data);

					$('input[name="menu_name"]').val(json.menu_name);
					$('select[name="menu_type"]').val(json.menu_type);
					$('input[name="view_url"]').val(json.view_url);
					$('input[name="event_key"]').val(json.event_key);
					$('input[name="listorder"]').val(json.listorder);
					$('input[name="menu_id"]').val(json.menu_id);
					$('select[name="pid"]').val(json.pid);
